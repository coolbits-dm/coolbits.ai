...
const cbFormatPlanPrice = (plan) => {
  const amount =
    typeof plan?.price?.amount === "number" ? plan.price.amount : null;
  const currency =
...
const cbRenderPlanAndUsageFromSummary = (summary) => {
  const planLabelEl = document.getElementById("cb-user-plan-label");
  const planTokensEl = document.getElementById("cb-user-plan-tokens");
  const sidebarPlanLabelEl = document.getElementById("cb-sidebar-plan-label");
  const sidebarPlanTokensEl = document.getElementById("cb-sidebar-plan-tokens");
  ...
  const sidebarSlot = document.getElementById("cb-sidebar-user-slot");
  const sidebarPlanButton = document.getElementById("cb-sidebar-plan-button");
  const planCode = cbNormalizePlanCode(summary?.plan);
  const fallbackLabel =
    PLAN_DEFINITIONS[planCode]?.label || "CoolBits Starter";
  const planLabel = summary?.plan?.label || fallbackLabel;
  ...
  const normalizedTokensFull =
    hasUsageTotals && summary
      ? `${formatTokens(Math.max(0, tokensRemaining))} / ${formatTokens(
          tokensIncluded,
        )} tokens`
      : "";
  const sidebarTokensText = summary
    ? hasUsageTotals
      ? normalizedTokensFull
      : "Tokens: n/a"
    : "Tokens: n/a";
  const tokensShort = cbFormatTokenShortText(tokensRemaining);

  if (planLabelEl) {
    planLabelEl.textContent = summary
      ? `Plan: ${planLabel}`
      : "Plan: Guest";
  }
  if (planTokensEl) {
    planTokensEl.textContent = summary ? tokensShort : "Tokens: n/a";
  }

  if (sidebarSlot) {
    sidebarSlot.hidden = false;
  }
  if (sidebarPlanButton) {
    sidebarPlanButton.disabled = false;
  }
  if (sidebarPlanLabelEl) {
    sidebarPlanLabelEl.textContent = summary ? planLabel : "Guest";
  }
  if (sidebarPlanTokensEl) {
    sidebarPlanTokensEl.textContent = sidebarTokensText;
  }
  if (headerPlanEl) {
...
const CB_MOBILE_BREAKPOINT = 960;
...
const cbResizeComposerInput = () => {
  ...
  const baseMax = lineHeight * CB_COMPOSER_MAX_LINES + paddingTop + paddingBottom;
  ...
  const baseMin = Math.max(cssMinHeight, lineHeight + paddingTop + paddingBottom);
  ...
  const rawTarget = Math.min(maxHeight, input.scrollHeight);
  const targetHeight = Math.max(minHeight, rawTarget);
...
const cbRenderCouncilActive = () => {
  ...
  container.innerHTML = "";
  const hasSelection = cbActiveCouncilMembers.length > 0;
  ...
  cbActiveCouncilMembers.forEach((member) => {
    const pill = document.createElement("div");
    pill.className = "cb-council-pill";

    const text = document.createElement("div");
    text.className = "cb-council-pill-text";

    const name = document.createElement("span");
    name.className = "cb-council-pill-name";
    name.textContent = member.label;

    const role = document.createElement("span");
    role.className = "cb-council-pill-role";
    role.textContent = member.badge;
    ...
    clear.type = "button";
    clear.className = "cb-council-pill-clear";
    clear.setAttribute("aria-label", `Remove ${member.label} council selection`);
    clear.textContent = "×";
    clear.addEventListener("click", () => {
      cbActiveCouncilMembers = cbActiveCouncilMembers.filter(
        (active) => !(active.id === member.id && active.workspaceId === member.workspaceId)
      );
...
const cbRenderCouncilList = () => {
  const listEl = shellElements.councilList;
  if (!listEl) {
    return;
  }
  const members = COUNCIL_BY_WORKSPACE[cbCurrentWorkspaceId] || [];
  cbActiveCouncilMembers = cbActiveCouncilMembers.filter(
    (member) =>
      member.workspaceId === cbCurrentWorkspaceId && members.some((item) => item.id === member.id)
  );
  listEl.innerHTML = "";
  members.forEach((member) => {
    const li = document.createElement("li");
    li.className = "cb-council-item";
    li.setAttribute("role", "option");
    li.dataset.councilId = member.id;

    const role = document.createElement("div");
    role.className = "cb-council-item-role";
    role.textContent = member.label;

    const badge = document.createElement("div");
    badge.className = "cb-council-item-badge";
    badge.textContent = member.badge;

    const desc = document.createElement("div");
    desc.className = "cb-council-item-desc";
    desc.textContent = member.desc;

    const isSelected = cbActiveCouncilMembers.some(
      (active) => active.id === member.id && active.workspaceId === cbCurrentWorkspaceId
    );
    if (isSelected) {
    ...
    li.addEventListener("click", () => {
      const alreadySelected = cbActiveCouncilMembers.some(
        (active) => active.id === member.id && active.workspaceId === cbCurrentWorkspaceId
      );
      if (alreadySelected) {
        cbActiveCouncilMembers = cbActiveCouncilMembers.filter(
        ...
        cbActiveCouncilMembers = cbActiveCouncilMembers.concat({
          ...member,
          workspaceId: cbCurrentWorkspaceId,
        });
      }
      cbRenderCouncilActive();
      cbRenderCouncilList();
...
async function cbCreateChat(firstMessage) {
  ...
  const payload = {
    ...
    councilMembers: cbActiveCouncilMembers || [],
  };
...
const setupSidebarInteractions = () => {
  const { sidebar, sidebarToggle, sidebarLogo, newChatButton } = shellElements;
  if (!sidebar) {
    return;
  }
  const storedCollapsed = loadSidebarCollapsedFromStorage();
...
const setupAccountBillingEntryPoints = () => {
  ...
  const sidebarButton = document.getElementById("cb-sidebar-plan-button");
  if (sidebarButton && sidebarButton.dataset.bound !== "true") {
...
const setupUserMenuHandlers = () => {
  ...
  const billingItem = document.getElementById("cb-user-menu-billing");
  ...
  if (billingItem) {
    billingItem.addEventListener("click", async () => {
      hideUserMenu();
      await cbOpenBilling();
    });
  }
...
console.log("[CB_CHAT_BUILD]", "v2025-11-30T21:00Z");
