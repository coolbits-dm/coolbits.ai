...
function cbGetLastReportStorageKey(connectorKey) {
  const workspaceId = cbGetWorkspaceStorageKey();
  return `cbx.lastReport.${workspaceId}.${connectorKey}`;
}

function cbGetConnectorLastReport(connectorKey) {
  try {
    const key = cbGetLastReportStorageKey(connectorKey);
    const raw = window.localStorage.getItem(key);
    return raw ? JSON.parse(raw) : null;
  } catch (error) {
    console.warn("[connectors] lastReport read failed", error);
    return null;
  }
}

function cbSetConnectorLastReport(connectorKey, payload) {
  try {
    const key = cbGetLastReportStorageKey(connectorKey);
    if (!payload) {
    ...
    console.warn("[connectors] lastReport write failed", error);
  }
}
  ...
  const baseReportBuilder = `
    ...
    <div class="cbx-section">
      ...
      <div class="cbx-row-compact" id="cbx-data-packs">
        ...
        <button class="cbx-chip" data-preset="campaign_performance" type="button">Campaign performance</button>
    ...
    <div class="cbx-section">
      <div class="cbx-section-title">Advanced</div>
      <div class="cbx-row-compact">
        <input class="cbx-input" type="search" placeholder="Search within results" disabled />
        <div class="cbx-log">Field selector coming soon.</div>
      </div>
    </div>
  `;
...
function cbBuildGoogleAdsReportSpec({ resource, dateRange, preset, compare }) {
  ...
  const selected = presets[preset] || presets.overview;
  return {
    resource,
    dateRange,
...
function cbBindGoogleAdsData(conn) {
  ...
  const stored = cbGetConnectorLastReport("googleads");
  if (stored) {
    cbRenderGoogleAdsStoredReport(stored);
  } else {
    const selectedAccount = cbGetConnectorStorage("googleads", "account", null);
    if (!selectedAccount?.customerId) {
      cbRenderGoogleAdsDataError("Select a client account in Setup.");
    } else {
      cbRenderGoogleAdsDataNote("No data yet. Click Fetch to run a report.");
    }
  }
}
...
function cbPersistGoogleAdsReportSpec(conn) {
  ...
  const preset = activeChip?.getAttribute("data-preset") || "overview";
...
function cbGetGoogleAdsReportParams() {
  ...
  return {
    ...
    preset: activeChip?.getAttribute("data-preset") || "overview",
  };
}
...
function cbRenderGoogleAdsKpis(overview, currency) {
  ...
  const items = [
    { label: "Spend", value: cbFormatCurrency(overview.cost, currency) },
    { label: "Impressions", value: cbFormatNumber(overview.impressions) },
    { label: "Clicks", value: cbFormatNumber(overview.clicks) },
    { label: "Conversions", value: cbFormatNumber(overview.conversions) },
    { label: "Conv. value", value: cbFormatCurrency(overview.convValue, currency) },
    { label: "CTR", value: cbFormatPercent(overview.ctr) },
    { label: "Avg CPC", value: cbFormatCurrency(overview.avgCpc, currency) },
    { label: "CPA", value: cbFormatCurrency(overview.cpa, currency) },
  ];
  items.forEach((item) => {
...
function cbRenderGoogleAdsCampaigns(rows, currency) {
  ...
  const sorted = rows
    .slice()
    .sort((a, b) => (Number(b.cost || 0) - Number(a.cost || 0)) || (Number(b.conversions || 0) - Number(a.conversions || 0)));
  sorted.slice(0, 10).forEach((row) => {
    ...
    name.textContent = `${row.name || "Unnamed campaign"}${channel}${status}`;
    const metrics = document.createElement("div");
    metrics.className = "cbx-log";
    const spend = cbFormatCurrency(row.cost, currency);
    const conversions = cbFormatNumber(row.conversions);
    const clicks = cbFormatNumber(row.clicks);
    const impressions = cbFormatNumber(row.impressions);
    metrics.textContent = `Imp ${impressions} · Clicks ${clicks} · Spend ${spend} · Conv ${conversions}`;
...
function cbIsGoogleAdsReportEmpty(overview, campaigns) {
  if (!overview && (!Array.isArray(campaigns) || !campaigns.length)) return true;
  const metrics = [
    Number(overview?.impressions || 0),
    Number(overview?.clicks || 0),
    Number(overview?.cost || 0),
    Number(overview?.conversions || 0),
    Number(overview?.convValue || 0),
  ];
  const hasAny = metrics.some((n) => n > 0);
  const hasCampaigns = Array.isArray(campaigns) && campaigns.length > 0;
  return !hasAny && !hasCampaigns;
}

function cbApplyGoogleAdsReport(payload, { note = "" } = {}) {
  if (!payload || typeof payload !== "object") return null;
  const data = payload.data || {};
  const currency = payload.currencyCode || payload.currency_code || "USD";
  const overview = data.overview || null;
  const campaigns = data.campaigns?.rows || [];
  cbRenderGoogleAdsKpis(overview, currency);
  cbRenderGoogleAdsCampaigns(campaigns, currency);
  if (note) {
    cbRenderGoogleAdsDataNote(note);
  } else if (payload.range?.from && payload.range?.to) {
    cbRenderGoogleAdsDataNote(`${payload.range.from} to ${payload.range.to}`);
  }
  const stored = {
    ts: Date.now(),
    ...
    customerId: cbConnectorState.googleads?.customerId || null,
    loginCustomerId: cbConnectorState.googleads?.loginCustomerId || null,
  };
  cbSetConnectorLastReport("googleads", stored);
  cbConnectorState.googleads.lastReport = payload;
  if (cbIsGoogleAdsReportEmpty(overview, campaigns)) {
    cbRenderGoogleAdsDataError("No data for selected range.");
  }
  return stored;
}

function cbRenderGoogleAdsStoredReport(stored) {
  if (!stored || typeof stored !== "object") return false;
  const payload = {
    range: stored.range || null,
    blocks: stored.blocks || [],
    data: stored.data || {},
    currencyCode: stored.currencyCode || "USD",
  };
  cbApplyGoogleAdsReport(payload, { note: "Last report loaded." });
  return true;
}

function cbMapGoogleAdsReportError(status, json) {
  if (status === 401) return "Sign in to view connectors.";
  if (status === 400 && json?.error === "manager_selected") {
    return "Selected account is a manager. Choose a client account in Setup.";
  }
  if (status === 400 && json?.error === "customer_not_set") {
    return "Select a client account in Setup.";
  }
  if (status === 403 || json?.error === "insufficient_permissions") {
    return "Insufficient permissions for selected client. Reconnect or grant access.";
  }
...
async function cbFetchGoogleAdsSnapshots() {
  ...
  snaps.forEach((snap) => {
    ...
    meta.textContent = `${snap.from} to ${snap.to}`;
    const loadBtn = document.createElement("button");
    loadBtn.type = "button";
    loadBtn.className = "cbx-btn";
    loadBtn.textContent = "Load";
    loadBtn.addEventListener("click", () => cbLoadGoogleAdsSnapshot(snap.id));
...
async function cbLoadGoogleAdsSnapshot(snapshotId) {
  const log = document.getElementById("cbx-conn-log");
  const out = document.getElementById("cbx-conn-output");
  if (!snapshotId) return;
  if (log) log.textContent = "Loading snapshot...";
  const { ok, status, json } = await cbFetchJson(`/api/connectors/googleads/snapshots/${snapshotId}`);
  if (!ok || !json?.snapshot?.payload) {
    if (log) {
      log.textContent = status === 401 ? "Sign in to view snapshots." : "Snapshot load failed.";
    }
    return;
  }
  const payload = json.snapshot.payload;
  cbApplyGoogleAdsReport(payload, {
    note: `Snapshot ${json.snapshot.label || snapshotId}`,
  });
  if (out) {
    const text = JSON.stringify(payload, null, 2);
    out.textContent = text.length > 8000 ? text.slice(0, 8000) + "\n…" : text;
  }
  if (log) log.textContent = `Snapshot loaded (#${snapshotId}).`;
}

async function cbSaveGoogleAdsSnapshot() {
  const log = document.getElementById("cbx-conn-log");
  const lastRun = cbConnectorState.googleads?.lastRun || null;
  const lastReport = cbConnectorState.googleads?.lastReport || null;
  const from = lastRun?.from || lastReport?.range?.from || null;
  const to = lastRun?.to || lastReport?.range?.to || null;
  const blocks = lastRun?.blocks || lastReport?.blocks || ["overview", "campaigns"];
  const compareMode = lastReport?.compare?.mode || null;
  const compareFrom = lastReport?.compare?.from || null;
  const compareTo = lastReport?.compare?.to || null;
  if (!from || !to) {
...
async function cbRunGoogleAdsReport({ source } = {}) {
  ...
  const out = document.getElementById("cbx-conn-output");
  if (cbNeedsConnectorsAuth()) {
    cbRenderGoogleAdsDataError("Sign in to view connectors.");
    if (log) log.textContent = "Sign in required.";
    return;
  }
  const selectedAccount = cbGetConnectorStorage("googleads", "account", null);
  const selectedCustomerId = cbConnectorState.googleads?.customerId || selectedAccount?.customerId || null;
  if (!selectedCustomerId) {
    cbRenderGoogleAdsDataError("Select a client account in Setup.");
    if (log) log.textContent = "Select a client account first.";
    return;
  }
  ...
  if (ok && json) {
    cbApplyGoogleAdsReport(json, { note: `${range.label} · ${preset}` });
    if (out) {
    ...
    cbRenderGoogleAdsDataError(cbMapGoogleAdsReportError(status, json));
...
// ESM + inline onclick compatibility (safe even if names don't exist).
(() => {
  ...
  defer(() => {
    ...
    expose("cbSignIn", () => cbSignIn);
    expose("cbOnCouncilModalClose", () => cbOnCouncilModalClose);
    expose("cbOnCouncilModalOpen", () => cbOnCouncilModalOpen);
    expose("cbOnCouncilPillClick", () => cbOnCouncilPillClick);
  });
})();
